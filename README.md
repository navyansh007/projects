...
{Insert greetings & stuff} 

Stuff to think about : 

The boilerplate project should be forked into codedamn-projects before merging the users project 

## How Can I Contribute?
You can contribute to this repository by adding your own projects or by adding new features to the existing projects.
...
1. Choose the Project that you want to contribute to, it could be a existing project on the repository or a personal project that you have built yourself. 

1. Create a fresh folder structure for your Project and keep the **required boilerplate code only**. Make sure to add all the assets related to the project. It is recommended to put all your assets  in the `/assets` folder itself. 

1. Add a `.cdmrc` file on the top level of your Project. Go through this [documentation](https://codedamn.com/news/product/how-to-use-codedamn-playgrounds#File_cdmrc) to setup the `.cdmrc` file.

1. Create a new GitHub repository for the Project and keep the link with you, you'll use it in the next step.

### 2. Creating your Project on Codedamn

1. Fork this repository and Clone it into your Computer. Open the repository in your favorite code editor.

1. Create a new folder in the Project. The name of the folder will be the project slug that will be displayed in the URL. The folder name should not contain any spaces, make use of `-` (Hyphen) between words. 
...
1. Create a `spec.yml` file inside the folder. This is the specification file that contains the metadata regarding the project. This data will be shown in the project landing page. 
    ...
    - `playground-layout` : This field is used to determine the layout of the playground. The possible values are `terminal-editor-browser`,`terminal-browser`, `editor-terminal`, `terminal`, 
...
4. The `Instructions.md` file will contain the instructions for the step. You can convey the details of what the user is supposed to implement in this step and guide them along with the way the specific details that the user should be aware of. 
	...
	//TODO : add instructions section image here 
...
5. The `challenges.yml` file will contain the challenges for the step. There are the clear cut deliverables that the user should implement in this step. The challenges are used to determine if the user has completed the step or not. 
	...
	* `stepBreakDown` : This field contains the list of all the challenges that this step holds. This is an Array of Objects in YAML. 
	...
	//TODO: add steps + step name + challenges tab image here
...
Once you have created a pull request, the project will be reviewed by the Codedamn team. 

// should we start merging the projects if they are ready, instead of waiting till 31st October?? 
